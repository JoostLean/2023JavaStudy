...
-메서드 혹은 변수명 : simpleFunc
	=> 소문자로 시작하는 Camel case로 작성한다.(이름 뒤에 괄호 있으면 메서드명 없으면 변수명)
-상수명 : SIMPLE_FUNC
	=> 전체를 대문자로 자성한다. 단 연결되는 단어는 _로 구분한다.
-패키지명 : simple.func
...
public class E02MethodType01 {
	...
	static void returnError() {
		...
		따라서 return은 반드시 조건문과 함께 기술되어야 한다. 단 메서드의
		마지막 라인에 기술한다면 조건문이 없어도 상관없다.
		*/
//		return;
		
		//이와 같이 return문이 중간에 있으려면 조건부로 기술해야 한다.
		if(returnValue%2==0) {
