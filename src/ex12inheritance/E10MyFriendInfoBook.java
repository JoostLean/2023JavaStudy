...
class HighFriend extends Friend {
	...
	*/
	...
	오버라이딩2 : 자식 클래스에서 오버라이딩하여 재정의한 메서드로 
		고딩친구의 간략한 정보를 출력한다. 부모클래스에서는 실행부가 
		없는 상태로 정의되었다.
	*/
	@Override
	public void showBasicInfo() {
...
public class E10MyFriendInfoBook {
	...
	정의한다. main메서드에서 직접 호출하기 위해 static으로 선언되어 
	있다. */
	...
	*/
	public static void main(String[] args) {
		...
		break문은 반복문을 탈출시키는 기능이 있으므로 이와 같은 무한루프에서 
		자주 사용한다.
...
/* 해당 프로그램에서 기능을 담당하는 클래스로 핸들러 혹은 매니져 클래스라고
부른다. */
class FriendInfoHandler {
	...
	public void addFriend(int choice) {
		...
		if(choice==1) {
			//고딩을 선택한경우 별명을 추가로 입력받는다.
			System.out.print("별명:"); iNickname = scan.nextLine();
			//HighFriend 인스턴스 생성후 참조변수에 저장한다.
			HighFriend high = new HighFriend(iName, iPhone, iAddr, iNickname);
			/* 참조변수를 인스턴스 배열에 추가한다. 인덱스로 사용된
			변수의 초기값이 0이므로, 0번 인덱스에 추가된 뒤 1증가된다.
			이를 위해 "후위증가" 하고 있다. */
	...
	1.친구정보를 추가할때 HighFriend 혹은 UnivFriend 인스턴스를 배열에 저장한다.
	...
	*/
	...
	public void showSimpleData() {
		...
		정의하지 않았다면, 자식클래스에서는 오버라이딩을 할수없으므로
		개별적인 멤버메서드를 정의해야한다.
		정보가 저장되는 배열은 Friend 타입이므로 자식인스턴스를 
		저장하면 자동으로 형변환(업캐스팅)이 된다. 이때 자식의 정보를
	...
	public void deleteInfo() {
		...
		Scanner scan = new Scanner(System.in);
		System.out.print("삭제할 이름을 선택하세요:");
		String deleteName = scan.nextLine();
		/* 배열의 원소 중 삭제된 원소의 인덱스값을 저장하기 위한 변수로
		삭제한 후 빈자리를 채워넣을때 사용한다. 인덱스는 0부터 시작이므로
		-1을 초기값으로 설정한다. */
		int deleteIndex = -1;

		//삭제할 인스턴스를 찾기위해 저장된 갯수만큼 반복한다.
		for(int i=0 ; i<numOfFriends ; i++) {
			//입력된 이름과 일치하는지 확인한다.
			if(deleteName.compareTo(myFriends[i].name)==0) {
				...
				실제로 참조되지 않는 인스턴스는 차후 "가비지 컬렉션"에
				의해 자동으로 삭제된다. */
				myFriends[i] = null;
				//삭제된 원소의 index를 저장한다.
				deleteIndex = i;
				//전체 카운트를 1 차감한다.
				numOfFriends--;
				//하나의 인스턴스 요소를 삭제하면 즉시 for문을 탈출한다.
				break;
			}
		}
		...
		if(deleteIndex==-1) {
			...
			/*
			인스턴스 배열에서 원소를 삭제한 후 바로 뒤에 있는 원소를
			앞으로 하나씩 복사한다. 만약 이 부분이 처리되지 않는다면
			차후 검색이나 삭제시 NullPointerException이 발생할 수
			있다.
			*/
