...
-데이터의 중복 저장이 허용된다.
-데이터의 저장 순서를 보장한다.
-Array라는 이름처럼 "배열"의 특성을 가지고 있어 index를 통한 저장 및
접근이 가능하다.
*/
		...
		*/
		...
		객체저장 : add()
			...
			add(인덱스, 인스턴스) : 데이터 저장시 인덱스를 직접
				부여한다. 단, 인덱스를 건너뛰면 에러가 발생하므로
				주의해야 한다. 만약 중복된 인덱스가 있다면 삽입(끼워넣기)
				이 된다.
		*/
		list.add("소녀시대");
		list.add("빅뱅");
...
//		list.add(5,"소방차ㅋㅋ"); //에러발생됨
		...
		*/
		...
		중복저장 : List는 배열의 특성을 가지므로 데이터를 중복저장
			할수있다. 동일한 데이터라도 index로 구분할 수 있기 때문이다.
		...
		*/
		...
		/*
		방법2 : 확장 for문의 경우 인덱스 없이 전체 원소를 반복해
			주므로 좀 더 간단한 표현이 가능하다. 따라서 가장 많이
			사용된다.
		*/
		System.out.println("\n[확장for문 사용]");
		for(String obj : list) {
		...
		*/
		...
		/*
		수정하기(덮어쓰기)
		: 기존의 내용을 변경한다. 즉 기존의 내용은 삭제한 후 새로운
		내용으로 갱신한다.
		*/
		...
		*/
		...
		/*
		포함여부확인
			contains(인스턴스) : 특정 인스턴스가 저장되어 있는지
				확인할때 사용한다. 존재한다면 true가 반환된다.
		*/
		...
		방법 : 인덱스로 삭제
			remove(인덱스) : 인덱스를 통해 삭제를 진행한 후
				삭제가 완료되면 해당 인스턴스를 반환한다. 삭제 후에는
				인덱스가 자동으로 부여된다. 삭제 성공시 삭제된 인스턴스를
		...
		*/
		...
		/*
		위치를 모르는 경우에는 indexOf()를 통해 해당 인스턴스의 
		인덱스를 찾은 후 삭제한다.
		*/
		...
		/*
		방법2 : 인스턴스의 참조값을 통해 삭제한다. 이 경우에는 삭제에 성공하면 
			true가 반환된다. 즉 boolean 타입의 반환값을 가진다.
		*/
