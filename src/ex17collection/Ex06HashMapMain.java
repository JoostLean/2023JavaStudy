...
public class Ex06HashMapMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		인스턴스 저장 : 최초 저장이라면 성공시 null을 반환한다. 만약
			기존에 이미 저장된 동일한 Key가 존재하면 저장된 인스턴스가
			반환된다.
		*/
		...
		/*
		중복저장 : 기존에 입력된 name이라는 Key가 있으므로 "홍길동"이
		...
		*/
		...
		/*
		출력하기
		1.Key를 알고있을때 : get(key) 으로 출력한다.
		*/
		System.out.println("키값을 알때:"+ map.get("name"));

		/*
		2.Key를 모를때(혹은 너무 많아서 하나씩 명시하기 힘들때)
		: keySet()을 통해 전체 Key를 얻어온다. 이를 통해 for문으로
		순차적인 접근을 하면 된다.
		*/
		...
		/*
		3.이터레이터 사용
		: 이터레이터를 통해 출력할때도 keySet()를 통해 전체 Key를
		얻어온 후 반복출력한다.
		*/
		...
		*/
		...
		/*
		인스턴스의 존재유무 확인
		: Map컬렉션은 Key와 Value가 있으므로 메서드도 아래와 같이
		2개로 나눠져 있다.
		*/
		...
		/*
		인스턴스 삭제 : Key를 통해 삭제하고, 삭제에 성공하면 해당 Value가
			반환된다.
		*/
