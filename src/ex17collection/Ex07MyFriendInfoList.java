...
//	private int numOfFriends;
	...
	*/
	...
	/*
	멤버변수가 변경되었으므로 생성자에서도 아래와 같이 수정해야 한다.
	컬렉션을 초기화 한다. 카운트가 필요없으므로 매개변수 num도 삭제한다. 
	*/
...
//			myFriends[numOfFriends++] = high;
			/* 기존 인스턴스 배열에서는 인덱스를 위한 카운트가 필요했지만
...
//	}
	...
	*/
	...
	public void deleteInfo() {
		...
		/*
		List컬렉션에 저장된 인스턴스를 기반으로 Iterator 인스턴스를
		생성한다. 이 때 타입매개변수는 List와 동일하게 정의하면 된다. 
		*/
		Iterator<Friend> itr = lists.iterator();
...
//		if(deleteName.compareTo(lists.get(deleteIndex).name) == -1)
		if(isFind == false) {
			System.out.println("==삭제된 데이터가 없습니다.==");
		}
		else {
