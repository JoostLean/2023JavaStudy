...
-기본클래스(String, Integer등)인 경우 별도의 오버라이딩 없이도
중복이 제거된다.
-하지만 개발자가 정의한 클래스라면 hashCode(), equals()를 적절히
오버라이딩해야만 중복을 제거할 수 있다.
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		int varInt = 100;//차후 저장시 Integer로 Boxing처리됨
		Teacher varTeacher = new Teacher("김봉두", 55, "체육");

		/*
		객체저장 : add()를 통해 저장한다. 만약 저장에 성공하면 true,
			실패하면 false를 반환한다.
		*/
		...
		/*
		기본클래스의 중복저장 : 기본 클래스를 통해 생성한 인스턴스인 경우
			별도의 오버라이딩 없이도 중복을 제거할 수 있다. 따라서 아래
			추가하는 부분에서는 false가 반환된다.
		 */
		...
		/*
		이터레이터를 통한 출력 : 순서없이 저장되므로 출력의 순서도 정할 수
			없다.
		*/
		...
		while(itr.hasNext()) {
			...
			/*
			Object형으로 자동형변환되어 저장되므로 우리가 직접 정의한
			클래스에 대해서만 instanceof를 통해 타입을 확인한 후
			다운캐스팅해준다. 
			*/
			if(object instanceof Teacher) {
				/* 다운캐스팅 하지 않으면 자식영역에 접근할 수 없으므로
				아래와 같이 getter를 호출할 수 없게 된다. */
		...
		/*
		포함여부 확인 : set컬렉션 내에 해당 인스턴스가 저장되어 있는지
			여부만 확인한다.
		*/
