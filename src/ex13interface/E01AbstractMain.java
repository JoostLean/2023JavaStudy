...
-인스턴스 생성은 할수없지만 그 외 참조변수, 상속 등의 모든 기능을 사용할 수
있다.
...
추상클래스2 : 추상메서드를 포함한 클래스, 추상메서드를 멤버로 포함한 클래스는
	반드시 abstract로 선언해야한다. 그렇지 않으면 에러가 발생한다.
*/
abstract class AbsClass2 {
	abstract void absMethod(int number);
}
/*
위 클래스를 상속하면 부모가 가진 추상메서드를 그대로 상속하게 되므로
해당 클래스도 abstract로 선언해야한다. 단, 이 경우 해당 클래스로도
인스턴스를 생성할 수 없게 된다. 따라서 인스턴스를 생성하기 위해서는 
부모의 추상메서드를 자식쪽에서 오버라이딩 해야한다. 
*/
class AbsClassChild2 extends AbsClass2 {
	/*
	오버라이딩은 부모쪽의 메서드를 재정의하는 것이므로 해당 클래스는 
	추상메서드를 포함하지 않은 상태가 된다.
	따라서 인스턴스 생성이 가능해진다.
...
public class E01AbstractMain {
	...
	public static void main(String[] args) {
		...
		//추상클래스는 인스턴스 생성은 안되지만, 참조변수로 사용할수있다.
		...
		없으므로 static의 기본규칙을 따른다. 별도의 생성절차 없이 
		클래스명으로 바로 접근할 수 있다. */
