...
/*
인터페이스(interface)
-클래스가 객체의 설계도라면 인터페이스는 클래스 간의 설계도라 할수있다.
-Java는 단일 상속을 원칙으로 하지만, 인터페이스는 다중 상속을 구현할 수 있다.
-추상클래스와 동일하게 상속을 목적으로 제작되므로 구현받은 하위클래스에서는 
추상메서드를 반드시 오버라이딩 해야한다.
-멤버로은 추상메서드와 정적상수만 선언할 수 있다.
	메서드 : public abstract
...
class MyClass extends SimpleClass 
	...
	하면된다. 오버라이딩은 항상 자식쪽의 메서드가 호출되므로 아무런 문제도
	발생되지 않는다.
	*/
	@Override
...
public class E03InterfaceMain {
	...
	public static void main(String[] args) {
		...
		즉, "상속", "구현" 모두 해당 클래스의 부모가 되므로 
		아래와 같이 참조변수로 선언할 수 있다.
		*/
		...
		my1.absFunc1();
		my1.absFunc2();
		//다른 부모클래스의 멤버에 접근하려면 캐스팅(형변환) 후 호출해야 한다.
		((SimpleClass)my1).mySimple();

		//삭제  /* MyClass의 부모는 모두 3개이므로 아래는 모두 참조가 가능하다. */
		//삭제  MyInterface2 my2 = new MyClass();
		//삭제  SimpleClass simpleClass = new MyClass();
	}
}
