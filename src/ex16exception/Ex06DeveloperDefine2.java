...
public class Ex06DeveloperDefine2 {
	...
	public static void main(String[] args) {
		/* main메서드에서는 나이를 입력받기 위한 메서드를 단순히
		호출만 한다. */
	...
	2차버전에서는 던지기를 하지 않고 해당 메서드에서 직접 예외처리를 한다. */
	public static int readAge() {
		...
		/*
		개발자가 직접 예외상황을 if문으로 확인한 후 예외클래스의 인스턴스를
		만들어 프로그램으로 throw한다. 그 즉시 에러가 발생하게 되므로
		호출한 지점으로 "예외던지기"를 하지 않고 여기서 직접 "예외처리"
		한다. 
		*/
		try {
			if(inputAge<0) {
