...
{
	/* 기본생성자를 통해 인스턴스를 생성하면 쓰레드명이
	JVM(자바가상머신)에 의해 자동으로 부여된다. */
	public NormalThread() {}
	...
	*/
	...
	/*
	쓰레드 동작 중 비정상적으로 종료될때 해당 예외가 발생할 수 있으므로
	예외처리를 하거나 아래와 같이 예외던지기를 해서 호출한 위치로 예외
	객체를 던질 수도 있다. 
	*/
	void threadMethod() throws InterruptedException {
		/*
		10번 반복중 2초씩 블럭상태가 되므로 총 20초간 실행하면서
		반복횟수와 쓰레드명을 출력한다. 
		*/
		for(int i=1 ; i<=10 ; i++) {
			System.out.printf("[%s]i=%d\n", getName(), i);
...
메인메서드도 하나의 독립쓰레드 이므로, 다른 독립쓰레드가 남아있다면
종료될 때까지 프로그램이 종료된 것이 아니다.
...
{
	/*
	무한루프로 구성되어 있고 별도의 탈출조건을 만들지 않는다. 3초에 한번씩
	블럭상태로 들어간다. 
	*/
	@Override
	public void run() {
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args)
	{
		...
		*/
		...
		/*
		첫번째 독립쓰레드 생성 및 시작. 우선순위를 별도로 부여하지
		않으므로 5로 지정된다. 쓰레드 인스턴스 생성시 이름을 부여하는
		방법으로 아래 2가지가 있다. 
		*/
		//방법1 : 생성자를 톨해 쓰레드 인스턴스 생성과 동시에 부여
		...
		/*
		두번째 독립쓰레드 생성 및 시작. 쓰레드명을 별도로 부여하지 않으면
		JVM이 자동으로 부여된다. 
		*/
