...
class ShowThread extends Thread
{
	...
	Thread 클래스의 run()을 오버라이딩 한것으로 직접 호출하면 안되고,
	start()를 통해 간접적으로 호출해야 한다. 만약 직접 호출하면 단순히
	실행만 되고 쓰레드가 생성되지 않는다. 
	*/
	@Override
	public void run() {
		//100번 반복 실행한다.
...
public class Ex01ThreadStart {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		run()을 직접 호출해서 실행하면 단순한 실행만 되고, 쓰레드가
		생성되지 않는다. 따라서 st1의 실행이 완료되야만 st2가 실행된다.
		즉 순차적인 실행이 된다. 
		*/
//		st1.run();
//		st2.run();
