...
//	}
	/*
	동기화 방법2
	: "동기화 블럭"을 사용한다. 동기화가 필요한 코드에만 적용하는 방법이다.
	해당 프로그램은 실행코드가 1줄밖에 없으므로 동기화 방법에 대한 성능차이가
	없다. 하지만 실행코드가 많은 경우에는 메서드 전체를 동기화 하는 것보다는
	필요한 코드만 동기화하는 "동기화블럭"이 훨씬 더 좋은 성능을 보이게 된다. 
	*/
