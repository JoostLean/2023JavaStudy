...
class Singer {
	...
	/* Object클래스에 정의된 메서드로 인스턴스가 가진 값을 문자열로
...
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		오버라이딩한 toString()의 반환값이 출력된다. 만약 toString을
		주석처리하면 @를 포함한 참조값이 출력된다. */
		...
		: Singer 타입의 변수에 GirlGroup을 상속받은 익명클래스를 
		생성해서 그 주소값을 할당한다.
		개념적으로 new GirlGroup(){}; => 익명 extends GirlGroup
		과 같다.
		...
		*/
		...
		접근하려면 다운캐스팅(강제형변환)을 해야하지만 익명클래스는
		이름이 없기 때문에 다운캐스팅 자체가 불가능하다.
		따라서 익명클래스에서는 멤버의 확장은 거의 사용하지 않는다.
		대부분 오버라이딩을 목적으로 활용된다. */
		//System.out.println(s3.mic); //에러발생됨.
		System.out.println(s3);

		/* 인터페이스를 부모로 사용하여 구현한 후 오버라이딩 하여
		기능을 재정의한 후 사용한다. */
